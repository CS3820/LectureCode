...
module Lecture3Full (module Lecture3Full) where
...
plusMToN :: Nat -> Nat -> Nat
plusMToN (Succ m) n = plusMToN (pred m) (Succ n)
plusMToN Zero n     = n

plusNToM :: Nat -> Nat -> Nat
plusNToM m Zero     = m
plusNToM m (Succ k) = let
  _ = m :: Nat
  _ = k :: Nat
  in
  plusNToM (Succ m) k
...
plusBoth :: Nat -> Nat -> Nat
plusBoth (Succ m) (Succ n) = Succ (Succ (plusBoth m n))
plusBoth (Succ m) Zero     = Succ (plusBoth m Zero)
plusBoth Zero     (Succ n) = Succ (plusBoth Zero n)
plusBoth Zero     Zero     = Zero
...
recNat :: (b -> b) -> b -> Nat -> b
recNat ifSucc ifZero Zero     = ifZero
recNat ifSucc ifZero (Succ n) = ifSucc (recNat ifSucc ifZero n)
